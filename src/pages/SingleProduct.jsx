...
function SingleProduct(){
    ...
    //Use navigate chai uata hoook ho jasle chai kun pani chiz lai kaha lane bhanera dekhaune ho
    ...
    const deleteMe = async()  =>{
        const response = await axios.delete("https://66dea33fde4426916ee1c965.mockapi.io/Products/" + id)
        if(response.status === 200){
            // setProduct(response.data)
            navigate("/")
            //Navigate le chai "/" bhanne ma lagidine if we delete the items
        }else{
            alert("Sorry, the items is not deleted so try again. Please.!!!")
        }
    }
        ...
        <div class="flex flex-col md:flex-row -mx-4">
            <div class="md:flex-1 px-4">
                <div class="h-[460px] rounded-lg bg-gray-300 dark:bg-gray-700 mb-4">
                    <img class="w-full h-full object-cover" src={product.prooductImage} alt="Product Image" />
                </div>
                <div class="flex -mx-2 mb-4">
                    <div class="w-1/2 px-2">
                    <Link to={`/edit/${product.id}`} >
                        <button class="w-full bg-gray-900 dark:bg-red-600 text-white py-2 px-4 rounded-full font-bold 
                        hover:bg-gray-800 dark:hover:bg-gray-700">Edit Items</button>
                    </Link>
                    </div>
                    <div class="w-1/2 px-2">
                        <button onClick= {deleteMe} class="w-full bg-gray-200 dark:bg-green-700 text-gray-800 dark:text-white py-2 px-4 rounded-full font-bold
                         hover:bg-gray-300 dark:hover:bg-gray-600">Delete Items</button>
                    </div>
                </div>
            </div>
            <div class="md:flex-1 px-4">
